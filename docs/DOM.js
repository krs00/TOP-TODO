...
import {
  ...
  updateProjectInfo,
} from "/modules/utils/projectManager";
...
export function openEditTaskModal(index) {
  ...
  const desc = document.querySelector("#task-description-input");
  const date = document.querySelector("#task-date-input");
  const obj = getCurrentTask(index);

  title.innerText = `*✩‧₊˚ edit ${obj.title} ✩ஃ`;
  name.value = obj.title;
  desc.value = obj.description;
  date.value = obj.dueDate;
}
